...
class AE2d(nn.Module):
    def __init__(self, in_channels, n_feat, sampling_config):
        super().__init__()
        ...
        down=[Down2d(2**i*n_feat, 2**(i+1)*n_feat, down=sampling_config[i]) for i in range(3)]
        self.down=nn.ModuleList(down)
        
        self.down_to_vec=nn.Sequential(nn.Conv2d(512,512,3,1,1),nn.AvgPool2d(sampling_config[-1]), nn.Tanh())
        self.vec_to_up=nn.Sequential(
            nn.ConvTranspose2d(2**3 * n_feat, 2**3 * n_feat, sampling_config[-1][0], sampling_config[-1][1]), 
            nn.GroupNorm(8, 2**3 * n_feat), nn.ReLU())
        up=[Up2d(2**(i+1)*n_feat, 2**i * n_feat, up=sampling_config[i]) for i in range(3)]
...
class SinusoidalEmbedding(nn.Module):
    def __init__(self, dim, scale=1.0):
        ...
        self.register_buffer(
            "freqs",
            torch.exp(
                -torch.arange(half_dim) * (np.log(10000) / (half_dim - 1))
            )
        )
...
class UNet(nn.Module):
    def __init__(self, in_channels, n_feat, feature_dim, sampling_config):
        ...
        down=[Down1d(2**i*n_feat, 2**(i+1) * n_feat, down=sampling_config[i]) for i in range(3)]
        self.down=nn.ModuleList(down)
        
        self.down_to_vec=nn.Sequential(nn.Conv1d(512,512,5,1,2),nn.AvgPool1d(sampling_config[-1]), nn.GELU())
        self.vec_to_up=nn.Sequential(
            nn.ConvTranspose1d(feature_dim + 2**3 * n_feat, 2**3 * n_feat, sampling_config[-1], sampling_config[-1]), 
            nn.GroupNorm(8, 2**3 * n_feat), nn.ReLU())

        up=[Up1d(2**(i+2)*n_feat, 2**i * n_feat, up=sampling_config[i]) for i in range(3)]
        self.up=nn.ModuleList(up)
    ...
    def forward(self, x, c, t, context_mask):
        ...
        temb = [self.temb[i](t)[:,:,None] for i in range(3)]  # (B, C, 1) for broadcasting
        ...
        vec = torch.cat([vec, c], dim=1)  # concat context || 512+512=1024
