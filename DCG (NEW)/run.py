...
def main(MR=[0.3]):
    ...
    if args.lambda_config is not None:
        applied = apply_lambda_config(config, args.lambda_config)
    ...
    for missingrate in MR:
        ...
        for data_seed in range(1, args.test_time + 1):
            ...
            set_run_seed(run_seed, use_cuda)

            x_train_list, mask = prepare_inputs(X_list, config['training']['missing_rate'], device)
            # Build the model
            ICDM = icdm(config)
            optimizer = build_optimizer(ICDM, config['training']['lr'])
            ...
            if not args.no_metrics:
                metrics_json_path = build_metrics_path(ckpt_root, dataset, missingrate, data_seed, 'json')
                metrics_csv_path = build_metrics_path(ckpt_root, dataset, missingrate, data_seed, 'csv')
                history_payload = {
                    'dataset': dataset,
                ...
                save_metrics_history_json(metrics_json_path, history_payload)
                save_metrics_history_csv(metrics_csv_path, history)
            ...
            if not args.no_checkpoint:
                last_ckpt = build_checkpoint_path(ckpt_root, dataset, missingrate, data_seed, 'last')
                save_checkpoint(last_ckpt, ICDM, optimizer, config, run_seed, data_seed, missingrate, run_metrics)

                if score >= best_score:
                    best_score = score
                    best_ckpt_path = build_checkpoint_path(ckpt_root, dataset, missingrate, data_seed, 'best')
                    save_checkpoint(best_ckpt_path, ICDM, optimizer, config, run_seed, data_seed, missingrate, run_metrics)
...
if __name__ == '__main__':
    ...
    MisingRate = parse_missing_rates(args.missing_rate)
